...
import com.upokecenter.text.encoders.*;
     ...
     * and other character inputs to sequences of bytes and vice versa. <p>
     ...
     * </p> <p> <b> Encoding Terms </b> </p> <ul> <li> A <b> code point </b>
     * is a number that identifies a single text character, such as a
     * letter, digit, or symbol. (A collection of such characters is also
     * called an <i> abstract character repertoire </i> .) </li> <li> A <b>
     * coded character set </b> is a set of code points which are each
     * assigned to a single text character. As used here, coded character
     * sets don't define how code points are laid out in memory. </li> <li>
     * A <b> character encoding </b> is a mapping from a sequence of code
     ...
     * example: </li> <li> (a) <code> UTF-16LE </code> and <code> UTF-16BE </code> are
     * two encodings defined in the Unicode Standard. They use 2 bytes for
     * the most common code points, and 4 bytes for supplementary code
     * points. </li> <li> (b) <code> UTF-8 </code> is another encoding defined in
     * the Unicode Standard. It uses 1 byte for ASCII and 2 to 4 bytes for
     * the other Unicode code points. </li> <li> (c) Most legacy East Asian
     * encodings, such as <code> Shift_JIS </code> , <code> GBK </code> , and <code> Big5
     * </code> use 1 byte for ASCII (or a slightly modified version) and,
     * usually, 2 or more bytes for national standard coded character sets.
     * In many of these encodings, notably <code> Shift_JIS </code> , characters
     * whose code points use one byte traditionally take half the space of
     * characters whose code points use two bytes. </li> <li> <b>
     * Escape-based encodings </b> are combinations of single- and/or
     * multi-byte encodings, and use escape sequences and/or shift codes to
     * change which encoding to use for the bytes that follow. For example:
     * </li> <li> (a) <code> ISO-2022-JP </code> supports several escape sequences
     * that shift into different encodings, including a Katakana, a Kanji,
     * and an ASCII encoding (with ASCII as the default). </li> <li> (b)
     * UTF-7 (not included in the Encoding Standard) is an encoding that
     ...
     * version of base-64) to encode other Unicode code points. </li> <li>
     * The Encoding Standard also defines a <b> replacement encoding </b> ,
     * which causes a decoding error and is used to alias a few problematic
     * or unsupported encoding names, such as <code> hz-gb-2312 </code> . </li>
     * </ul> <p> <b> Getting an Encoding </b> </p> <p> The Encoding Standard
     * includes UTF-8, UTF-16, and many legacy encodings, and gives each one
     * of them a name. The <code> GetEncoding(name) </code> method takes a name
     * string and returns an ICharacterEncoding object that implements that
     * encoding, or <code> null </code> if the name is unrecognized. </p> <p>
     * However, the Encoding Standard is designed to include only encodings
     * commonly used on Web pages, not in other protocols such as email. For
     * email, the Encoding class includes an alternate function <code>
     * GetEncoding(name, forEmail) </code> . Setting <code> forEmail </code> to <code>
     * true </code> will use rules modified from the Encoding Standard to
     * better suit encoding and decoding text from email messages. </p> <p>
     * <b> Classes for Character Encodings </b> </p> <p> This Encodings
     * class provides access to common character encodings through classes
     * as described below: </p> <ul> <li> An <b> encoder class </b> is a
     * class that converts a sequence of bytes to a sequence of code points
     * in the universal character set (otherwise known under the name
     * Unicode). An encoder class implements the <code> ICharacterEncoder </code>
     * interface. </li> <li> A <b> decoder class </b> is a class that
     * converts a sequence of Unicode code points to a sequence of bytes. A
     * decoder class implements the <code> ICharacterDecoder </code> interface.
     * </li> <li> An <b> encoding class </b> allows access to both an
     * encoder class and a decoder class and implements the <code>
     * ICharacterEncoding </code> interface. The encoder and decoder classes
     * should implement the same character encoding. </li> </ul> <p> <b>
     * Custom Encodings </b> </p> <p> Classes that implement the
     ...
     * version of UTF-7 used in the IMAP email protocol. </li> </ul> <p>
     * (Note that this library doesn't implement either encoding.) </p>
     */
...
ICharacterEncoding enc,
  byte[] bytes) {
     ...
     * implementing ICharacterEncoding and can be called as follows: <code>
     * enc.DecodeToString(bytes, offset, length) </code> . If the object's
...
ICharacterEncoding enc,
  ...
  int length) {
     ...
     * <code> input.EncodeToBytes(encoding) </code> . If the object's class
...
ICharacterInput input,
  ICharacterEncoder encoder) {
     ...
     * htmlFallback) </code> . If the object's class already has a <code>
     * EncodeToBytes </code> method with the same parameters, that method takes
     * precedence over this extension method. </p>
     * @param input An object that implements a stream of universal code points.
...
String str,
  ...
  false);
     ...
     * characters are replaced with the replacement character (U + FFFD). <p>
...
ICharacterInput input,
  ...
  IWriter writer) {
     ...
     * implementation, this method is implemented as an extension method to
     * any object implementing ICharacterInput and can be called as follows:
     * <code> input.EncodeToWriter(encoder, writer) </code> . If the object's
...
ICharacterInput input,
  ...
  IWriter writer) {
     ...
     * str.EncodeToBytes(enc, writer) </code> . If the object's class already
     * has a EncodeToBytes method with the same parameters, that method
     * takes precedence over this extension method. </p> <p> In the .NET
     * implementation, this method is implemented as an extension method to
     * any object implementing string and can be called as follows: <code>
     * str.EncodeToWriter(enc, writer) </code> . If the object's class already
...
ICharacterInput input,
  ...
  OutputStream output) throws java.io.IOException {
     ...
     * implementation, this method is implemented as an extension method to
     * any object implementing ICharacterInput and can be called as follows:
     * <code> input.EncodeToWriter(encoder, output) </code> . If the object's
     * class already has a <code> EncodeToWriter </code> method with the same
     * parameters, that method takes precedence over this extension method.
     * </p>
     * @param input An object that implements a stream of universal code points.
...
ICharacterInput input,
  ...
  OutputStream output) throws java.io.IOException {
     ...
     * implementation, this method is implemented as an extension method to
     * any object implementing string and can be called as follows: <code>
     * str.EncodeToWriter(enc, output) </code> . If the object's class already
     * has a <code> EncodeToWriter </code> method with the same parameters, that
     * method takes precedence over this extension method. </p>
     * @param str A text string to encode.
     * @param enc An object implementing a character encoding (gives access to an
...
ICharacterEncoding encoding,
  InputStream input) {
     ...
     * Encoding standard. </p> <p> In the .NET implementation, this method
     * is implemented as an extension method to any object implementing
     * ICharacterEncoding and can be called as follows: <code>
     * encoding.GetDecoderInputSkipBom(input) </code> . If the object's class
...
ICharacterInput reader) {
     ...
     * changing aliases of a character encoding to a standardized name. <p>
     * In several Internet specifications, this name is known as a "charset"
     ...
     * encoding {@code x-user-defined } </li> <li> The special-purpose
     * encoding {@code replacement } . </li> <li> 28 legacy single-byte
     * encodings: <ul> <li> {@code windows-1252 } : Western Europe (Note:
     * The Encoding Standard aliases the names {@code US-ASCII } and {@code
     * ISO-8859-1 } to {@code windows-1252 } , which uses a different coded
     * character set from either; it differs from {@code ISO-8859-1 } by
     ...
     * pages.) </li> <li> {@code ISO-8859-2 } , {@code windows-1250 } :
     * Central Europe </li> <li> {@code ISO-8859-10 } : Northern Europe
     * </li> <li> {@code ISO-8859-4 } , {@code windows-1257 } : Baltic </li>
     * <li> {@code ISO-8859-13 } : Estonian </li> <li> {@code ISO-8859-14 }
     * : Celtic </li> <li> {@code ISO-8859-16 } : Romanian </li> <li> {@code
     * ISO-8859-5 } , {@code IBM-866 } , {@code KOI8-R } , {@code
     ...
     * {@code Shift_JIS } , {@code EUC-JP } , {@code ISO-2022-JP } </li>
     * <li> Two legacy simplified Chinese encodings: {@code GBK } and {@code
     * gb18030 } </li> <li> {@code Big5 } : legacy traditional Chinese
     * encoding </li> <li> {@code EUC-KR } : legacy Korean encoding </li>
     * </ul> <p> The {@code UTF-8 } , {@code UTF-16LE } , and {@code
     * UTF-16BE } encodings don't encode a byte-order mark at the start of
     * the text (doing so is not recommended for {@code UTF-8 } , while in
     * {@code UTF-16LE } and {@code UTF-16BE } , the byte-order mark
